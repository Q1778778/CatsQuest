...
let parse  c=
  ...
  |Command s-> cplace.dialog_in_progress<-true;dialog "this is a GUI tester" Color_convert.cute_cat "cute cat"
...
let ksensor sta=
  let key=sta.key in 
  match key with 
  |'a'->Engine.move_player_right Engine.game_state; cplace.irefresh<-true
  |'d'->Engine.move_player_left Engine.game_state; cplace.irefresh<-true
...
let game_over_mon ()=
  ...
  if h<=0 then (  health_bar ();
                  dialog "Game over" Color_convert.cute_cat "cute cat"; tsensor cplace; Graphics.close_graph()) else ()
